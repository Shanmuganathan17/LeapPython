...
lstReofferKeyword = ['reoffer', 're-offer', 're_offer', 'reproposal', 're-proposal', 're_proposal']
doc_Count = 0
doc_Path = []
...
try:
    ...
    # SQL query to retrive datafrom lateral offer table for duplicate candidate id check
    lateralOffer_query = 'SELECT * FROM lateralOffer'

    # Using pandas to read data from SQL into a DataFrame
    df = pd.read_sql_query(sql_query, leap_connection)
    df_LateralOffer = pd.read_sql_query(lateralOffer_query, leap_connection)
    lstCandidateId = list(set(df_LateralOffer['candidateId'].values.tolist()))
    logger.info(lstCandidateId)
...
def ODS_Extraction(my_dict, attachmentCount):
    logger.info(my_dict)
    global final_dict
    # final_dict={}
    ...
    # print(len(my_dict['Doc File Path']))
    # docFilePath = [inner_dict['Doc File Path'] for outer_key, inner_dict in my_dict.items()]
    # print(docFilePath)
    ...
    for i in range(attachmentCount):
        ...
        try:
            ...
            attachmentPath = my_dict['Doc File Path']
            my_text = docx2txt.process(attachmentPath[i])  # reading ODS file as a string
            # print('docx2txt' + my_text)
            document = Document(my_dict['Doc File Path'][i])  # reading ODS file as a tables format
            # print('file ppath' + document)
            phrase = re.sub(r'\s+', ' ',
                            my_text)  # converting whole multiple lines in to one single line without \n spaces
            extractedTextContent.append(phrase)
            extractedTableContent.append(document)
            logger.info('Reading doc and mapping file - Completed')
        except Exception as e:
            logger.error('ERROR in doc and mapping file : ' + str(e))
            my_check = False
            extractedTextContent.append(None)
            extractedTableContent.append(None)
        ...
        try:
            ...
            if my_dict['Candidate ID ODS Field'] in phrase and my_dict['Indent number ODS'] in phrase and my_dict[
                ...
                'SEZ ODS'] in phrase and my_dict['STP ODS'] in phrase and my_dict['Skill Group ODS'] in phrase and \
                    my_dict[
                ...
                'Restart With Infosys ODS'] in phrase and my_dict[
                'Subcon ID for subcon conversion cases ODS'] in phrase and \
        ...
        docFormatMatchCheck.append(my_check)
    print('Hi')
    print(sum(docFormatMatchCheck))

    if sum(docFormatMatchCheck) > 1:
        ...
        my_check = False
    elif sum(docFormatMatchCheck) == 1:
        templateIndexPostion = docFormatMatchCheck.index(True)
        phrase = extractedTextContent[templateIndexPostion]
        document = extractedTableContent[templateIndexPostion]
        ...
        try:
            ...
            for i in range(len(column_list)):
                    ...
                    unitname=str(find_between_r(phrase, my_dict['allocated to unit ods'], my_dict['account name ods'])).strip()
                    if unitname in column_list:
                        print(unitname)
                    if str(find_between_r(phrase, my_dict['allocated to unit ods'],my_dict['account name ods'])).strip().lower() == str(column_list[i]).lower():
                        final_dict.update({'practice unit': str(find_between_r(phrase, my_dict['allocated to unit ods'], my_dict['account name ods'])).strip()})
            ...
            if final_dict['Practice Unit'] == None:
                logger.error('Allocate to (Unit) criteria check : Failed ' + str(find_between_r(phrase, my_dict['Allocated to Unit ODS'],my_dict['Location of Posting ODS'])).strip())
        except Exception as e:
        ...
        # Allocate to (Unit) criteria check if that string is mapped into the Unit Mapping column or not
        try:
            column_list = ColumnsData(df, my_dict['Unnamed 2 Column'])
            for i in range(len(column_list)):
                if str(find_between_r(phrase, my_dict['Allocated to Unit ODS'], 'Account Name')).strip().lower() == str(
                        column_list[i]).lower():
                    final_dict.update({'Practice Unit': str(
                        find_between_r(phrase, my_dict['Allocated to Unit ODS'], 'Account Name')).strip()})
                    my_dict['pUCode Column'] = 'pUCode'
                    final_dict.update({'Unit': df.iloc[i][my_dict['pUCode Column']]})
                    break
                else:
                    final_dict.update({'Practice Unit': None})
...
def parse_attachments(attachments, mailId):
    ...
    for attachment in attachments:
        ...
        print('Doc File Name :' + os.path.join(os.getcwd(), filename))

        if attach['extension'] == '.zip':
            filename = os.path.join(os.getcwd(), filename)
            # filename=r'C:\Users\shanmuganathan.s\Downloads\Pavan G_1006977795 (1)\Pavan G_1006977795.zip'
            ...
            with ZipFile(filename, 'r') as f:
                ...
                for elem in fileNames:
                    if '.docx' in elem.filename and '.doc' in elem.filename:
                        ...
                        # destination=r'C:\Users\shanmuganathan.s\Downloads\Pavan G_1006977795 (1)\Pavan G_1006977795\ODS Version 11.docx'
                        doc_Path.append(destination)
                        # my_dict['Doc File Path'].append(destination)
            f.close()
            ...
            # my_dict['Doc File Path'].append(os.path.join(os.getcwd(), str(filename)))
            destination = os.path.join(os.getcwd(), str(filename))
            doc_Path.append(destination)
        ...
        if (response.status_code == 200):
            ...
            logger.info('Attachment: ' + str(attachmentLink['fileid']))
            # logger.info(attach['contentID'])
        else:
            attachmentLink['filename'] = filename
    ...
    my_dict['Doc File Path'] = doc_Path
        ...
        if caseIdRegex.search(subject) != None:
            ...
            for keyword in keywordToProcess.keys():
                ...
                if keyword in subject.lower():
                    ...
                    if doc_Count == 1:
                        try:
                            ODS_Extraction(my_dict, doc_Count)
                            print(final_dict)
                    ...
                    elif not any(final_dict.values()) or doc_Count > 1:
                        try:
                            final_dict = ODS_Extraction(my_dict, doc_Count)
                            print(final_dict)
                            row['candidateName'] = final_dict['Candidate Name']
                            row['accountName'] = final_dict['Account Name']
                            row['rwi'] = final_dict['Restart With Infosys']
                ...
                if final_dict['Candidate ID'] in lstCandidateId:
                    canidCheck = True
                else:
                    canidCheck = False

                if any(keyword.lower() in subject.lower() for keyword in lstReofferKeyword):
                    reOfferKeywordCheck = True
                else:
                    reOfferKeywordCheck = False

                # Candidate ID Exsist Check
                if final_dict['Candidate ID'] in lstCandidateId and 'reoffer' not in subject.lower():
                    print(
                        'Candidate ID All ready present, may be response mail, data needs to update for available case ')
                elif final_dict['Candidate ID'] in lstCandidateId and 'reoffer' in subject.lower():
                    print('reoffer mail- new case to be created')
                elif final_dict['Candidate ID'] not in lstCandidateId:
                    print('fresh mail for case creation')

                print(final_dict['Practice Unit'])
                print(lst_UnitsForCaseCreation)
                # final_dict['Practice Unit'] in lst_UnitsForCaseCreation
                if  doc_Count > 0 and not canidCheck and not reOfferKeywordCheck and final_dict['Practice Unit'] in lst_UnitsForCaseCreation:
                #if doc_Count > 0:
